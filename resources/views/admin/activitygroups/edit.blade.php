...
<a href="{{ url('admin/activitygroup') }}" class="btn btn-default btn-sm backlink"><i class="fa fa-list" aria-hidden="true"></i> Show List </a>
@endsection

@section('content')
<form class="form-horizontal" role="form" action="{{ url('admin/activitygroup/'.$data->id) }}" method="post" enctype="multipart/form-data">
  {{ csrf_field() }}    
  ...
  <div class="col-md-9">
    ...
    <div class="panel-body">
      ...
      <div class="form-group">
        ...
        <div class="col-lg-9">
          <div class="bs-component">
            <input type="text" id="" name="meta_keyword" class="form-control" value="{{$data->meta_keyword}}" placeholder="Meta Keyword" />
          </div>
        </div>
      </div>
    ...
    <div class="sid_bvijay mb10">
      ...
      <div class="hd_show_con">
        ...
        <span class="id{{$data->id}}">
          ...
          <img src="{{asset(env('PUBLIC_PATH').'uploads/banners/'.$data->banner)}}" width="30%" />
          <hr/>
        </span>     
        @endif
        <div id="xedit-demo">
...
<script type="text/javascript">
  ...
  $('.imagedelete').on('click',function(e){
    ...
    $.ajax({
      type:'delete',
      url:"{{url('delete_activity_banner') . '/'}}" + id,
      data:{_token:csrf},    
