...
@section('breadcrumb')
<a href="{{ url('admin/schedule/'.Request::segment(3).'/create') }}" class="btn btn-primary btn-sm">Create </a>
@endsection
          ...
          <tbody>
            ...
            <tr class="id{{$row->id}}">
              ...
              <td class="title_hi_sh">              
                 <strong> {{ ucfirst($row->start_date) }} </strong> 
               <div class="row_actions"><span class="id">ID: {{$row->id}} | </span><span class="edit"><a href="{{url( 'admin/schedule/'.Request::segment(3).'/'.$row->id.'/edit')}}" aria-label="">Edit</a> 
                 </span>                 
...
(function ( $ ) { 
  $('.submitdelete1').on('click',function(e){
    ...
    $.ajax({
      type:'delete',
      url:"{{url('admin/schedule').'/'.Request::segment(3).'/' }}" + id,
      data:{_token:csrf},    
