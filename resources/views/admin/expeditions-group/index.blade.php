...
@section('title', '')
@section('breadcrumb')
<!--<a href="{{ route('expedition-group.create') }}" class="btn btn-primary btn-sm">Create</a>-->
@endsection
@section('content')
				...
				<table class="table admin-form theme-warning fs13">
						...
						<tbody>
							...
							<tr class="id{{$row->id}}">
								...
								<td class="">{{ ucfirst($row->group_title) }}</td>
								...
								<td class="text-left">
									<a href="{{ route('expedition-group.edit',$row->id) }}">Edit</a>
...
jQuery(document).ready(function() {
  $('.btn-delete').on('click',function(e){
    ...
    var url = '{{ route("expedition-group.destroy",":id") }}';
  ...
  $('.CheckStatus').on('click',function(e){
    ...
    var url  = '{{ route("admin.expgroupstatus",["id"=>':id']) }}';
      url = url.replace(':id',id);
      $.ajax({
        type:'put',
        url:url,
