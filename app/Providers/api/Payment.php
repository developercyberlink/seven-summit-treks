...
use App\Http\ActionRequest;
use App\Http\SecurityData;
...
class Payment extends ActionRequest
{
    ...
    public function ExecuteFormJose($mid,$api_key,$curr,$amt,$threeD,$success_url,$failed_url,$cancel_url,$backend_url, $book): string
    {
        $now = Carbon::now();
        $orderNo = $now->getPreciseTimestamp(3);
        $request = [
            ...
            "officeId" => $mid,
            "orderNo" => $orderNo,
            "productDescription" => $book->trip_id ,
            "paymentType" => "CC",
            "paymentCategory" => "ECOM",            
            "storeCardDetails" => [
                "storeCardFlag" => "N",
            ...
            "purchaseItems" => [
                [
                    "purchaseItemType" => "ticket",
                    "referenceNo" => $orderNo,
                    "purchaseItemDescription" => "Booking for ". $book->trip_id ? $book->trip_id : 'Trip Not Found!',
                    "purchaseItemPrice" => [
                        "amountText" => str_pad(($amt==null?0:$amt)*100,12,"0",STR_PAD_LEFT),
                        "currencyCode" => $curr,
                        "decimalPlaces" => 2,
